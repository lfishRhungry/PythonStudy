...
def process_analyze_show_data(data_df):
    ...
    for i, feat_col in enumerate(feat_cols):
        ...
        y = data_df['Y'].values                       # 标签多维数组只要是一维向量就ok
        ...
        # 当前子图画散点图和直线(折线图)
        axes[(i + 1) // 2 - 1, i % 2].scatter(X, y, alpha=0.5)
        axes[(i + 1) // 2 - 1, i % 2].plot(X, w * X + b, c='r')
        axes[(i + 1) // 2 - 1, i % 2].set_xlabel(f'{feat_col}')
        axes[(i + 1) // 2 - 1, i % 2].set_ylabel('Y指标')
        axes[(i + 1) // 2 - 1, i % 2].set_title(f'{feat_col}预测情况')
