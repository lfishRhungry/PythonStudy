# 利用线性回归模型实现房价预测 画出各个特征与标签的散点图观察规律

# 线性回归算法基础 最小二乘法 线性回归模型
...
def plot_feat_and_price(data_df):
    ...
    for i, feat_col in enumerate(feat_cols):
        # 提取画图需要的一个特征列和标签列  画散点图                                     标明画在哪一个子图上
        data_df[[feat_col, 'price']].plot.scatter(x=feat_col, y='price', alpha=0.5, ax=axes[int(i / 3), i - 3 * int(i / 3)])
    plt.tight_layout()
    plt.show()
